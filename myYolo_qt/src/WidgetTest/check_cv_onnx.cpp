...
bool Check_CV_Onnx::checkOnePicture(const QString &inputPath, const QString &outputPath)
{
    ...
    cv::Mat blob = cv::dnn::blobFromImage(resized_img, 1 / 255.0, cv::Size(m_info.model_width, m_info.model_height),
    ...
    // NMS box
    cv::dnn::NMSBoxes(m_info.boxes, m_info.confidences, m_info.confidence_threshold, m_info.nms_threshold,
                      m_info.nms_indices);
...
cv::Mat Check_CV_Onnx::doDrawBox(const cv::Mat &img)
{
    ...
    for (int idx : m_info.nms_indices)
    {
        ...
        oss_box << std::fixed << std::setprecision(2) << "{" << box.x << ", " << box.y << ", " << box.width << ", "
                << box.height << "}";
        ...
        cv::putText(img_copy, info_base, pos_base, cv::FONT_HERSHEY_SIMPLEX, 0.5, COLORS.at(m_info.class_ids.at(idx)),
                    2);
