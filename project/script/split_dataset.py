...
def split_dataset(dir_in, dir_out, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1):
    # 用于存储所有符合条件的文件路径
    all_files = []
...
def write_to_file(file_path, files):
    with open(file_path, 'w', encoding='utf-8') as f:
        for file in files:
            # 获取文件的绝对路径并写入文件
            absolute_path = os.path.abspath(file)  # 获取绝对路径
...
def main():
    ...
    # 调用函数进行数据集拆分
    split_dataset(args.dir_in, args.dir_out, args.train_ratio, args.val_ratio, args.test_ratio)
