...
usage:
    # python xml2txt.py -h
    python xml2txt.py --class_name your_classname --input_folder your_input --output_folder your_output
'''
import os
import argparse
import xml.etree.ElementTree as ET

def convert_xml_to_yolo(class_name, xml_file, output_file):
    ...
    with open(output_file, 'w') as out_file:
        for obj in root.iter('object'):
            ...
            class_id = class_name.index(class_name)  # 获取类别 ID
...
def convert_folder_xml_to_yolo(class_name, input_folder, output_folder):
    for xml_file in os.listdir(input_folder):
        if xml_file.endswith('.xml'):
            xml_path = os.path.join(input_folder, xml_file)
            txt_file = xml_file.replace('.xml', '.txt')
            output_path = os.path.join(output_folder, txt_file)
            convert_xml_to_yolo(class_name, xml_path, output_path)
...
def main():
    ...
    # 添加输入和输出路径的命令行参数
    parser.add_argument('--class_name', type=str, required=True, help='name of the class')
    ...
    # 运行转换
    convert_folder_xml_to_yolo(args.class_name, args.input_folder, args.output_folder)
