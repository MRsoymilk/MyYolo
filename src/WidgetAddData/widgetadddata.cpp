...
WidgetAddData::WidgetAddData(QWidget *parent)
    : QWidget(parent), ui(new Ui::WidgetAddData) {
  ui->setupUi(this);
  initAddData();
}
...
void WidgetAddData::getCfgData() {
  ...
  QStringList tags =
      SETTING_GET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_LIST_TAG).split(',');
  tags.removeAll("");
  ...
  m_data.dir_output_divide =
      SETTING_GET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_DIR_OUTPUT_DIVIDE);
  m_data.is_shuffle =
      SETTING_GET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_IS_SHUFFLE).toInt();
  m_data.is_rename =
      SETTING_GET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_IS_RENAME).toInt();
  m_data.rename_prefix =
      SETTING_GET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_RENAME_PREFIX);
  m_data.rename_suffix =
      SETTING_GET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_RENAME_SUFFIX);
  m_data.is_rename_no_tag =
      SETTING_GET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_IS_RENAME_NOTAG).toInt();
  m_data.re_notag_prefix =
      SETTING_GET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_RE_NOTAG_PREFIX);
  m_data.re_notag_suffix =
      SETTING_GET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_RE_NOTAG_SUFFIX);
}
...
void WidgetAddData::show2Ui() {
  ...
  ui->checkBoxShuffle->setCheckState(m_data.is_shuffle ? Qt::Checked
                                                       : Qt::Unchecked);
  ui->checkBoxRename->setCheckState(m_data.is_rename ? Qt::Checked
                                                     : Qt::Unchecked);
  ui->lEditRenamePrefix->setText(m_data.rename_prefix);
  ui->lEditRenameSuffix->setText(m_data.rename_suffix);
  ui->checkBoxReNoTag->setCheckState(m_data.is_rename_no_tag ? Qt::Checked
                                                             : Qt::Unchecked);
...
void WidgetAddData::save2Cfg() {
  ...
  SETTING_SET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_LIST_TAG,
              m_data.list_tag.join(','));
  SETTING_SET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_TRAIN,
              QString::number(m_data.train));
  SETTING_SET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_VALID,
              QString::number(m_data.valid));
  SETTING_SET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_TEST,
              QString::number(m_data.test));
  SETTING_SET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_DIR_OUTPUT_DIVIDE,
              m_data.dir_output_divide);
  SETTING_SET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_IS_SHUFFLE,
              QString::number(m_data.is_shuffle));
  SETTING_SET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_IS_RENAME,
              QString::number(m_data.is_rename));
  SETTING_SET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_RENAME_PREFIX,
              m_data.rename_prefix);
  SETTING_SET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_RENAME_SUFFIX,
              m_data.rename_suffix);
  SETTING_SET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_IS_RENAME_NOTAG,
              QString::number(m_data.is_rename_no_tag));
  SETTING_SET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_RE_NOTAG_PREFIX,
              m_data.re_notag_prefix);
  SETTING_SET(CFG_GROUP_ADD_DATA, CFG_ADD_DATA_RE_NOTAG_SUFFIX,
              m_data.re_notag_suffix);
...
void WidgetAddData::callXml2Txt() {
  ...
  QStringList arguments_xml2txt{
      GLOBAL.SCRIPT_XML2TXT, "--class_name", m_data.list_tag.join(','),
...
void WidgetAddData::callNoXml2Txt() {
  ...
  QStringList arguments_no_xml2txt{GLOBAL.SCRIPT_NO_XML2TXT, "--input_folder",
                                   m_data.dir_img, "--output_folder",
                                   m_data.dir_img};
...
void WidgetAddData::callRename() {
  QStringList arguments_rename{GLOBAL.SCRIPT_RENAME, "--input_folder",
                               m_data.dir_img};
...
void WidgetAddData::callReNoTag() {
  QStringList arguments_re_notag{GLOBAL.SCRIPT_RE_NOTAG, "--input_folder",
                                 m_data.dir_img};
...
void WidgetAddData::on_btnStartAddData_clicked() {
  ...
  if (ui->checkBoxRename->isChecked()) {
    if (ui->lEditRenamePrefix->text().isEmpty() &&
        ui->lEditRenameSuffix->text().isEmpty()) {
  ...
  if (ui->checkBoxReNoTag->isChecked()) {
    if (ui->lEditReNoTagPrefix->text().isEmpty() &&
        ui->lEditReNoTagSuffix->text().isEmpty()) {
      WIDGET_LOG_WARN("empty prefix and suffix");
    } else {
      callReNoTag();
    }
