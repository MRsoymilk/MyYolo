...
WidgetExport::WidgetExport(QWidget *parent)
    : QWidget(parent), ui(new Ui::WidgetExport) {
  ui->setupUi(this);
  initExport();
}
...
void WidgetExport::getCfgData() {
  ...
  m_data.model_batch =
      SETTING_GET(CFG_GROUP_EXPORT, CFG_EXPORT_MODEL_BATCH).toInt();
}
...
void WidgetExport::save2Cfg() {
  ...
  SETTING_SET(CFG_GROUP_EXPORT, CFG_EXPORT_MODEL_H,
              QString::number(m_data.model_h));
  SETTING_SET(CFG_GROUP_EXPORT, CFG_EXPORT_MODEL_W,
              QString::number(m_data.model_w));
  SETTING_SET(CFG_GROUP_EXPORT, CFG_EXPORT_MODEL_BATCH,
