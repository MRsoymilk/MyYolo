...
WidgetFilter::WidgetFilter(QWidget *parent)
    : QWidget(parent), ui(new Ui::WidgetFilter) {
  ui->setupUi(this);
  initFilter();
}
...
void WidgetFilter::getCfgData() {
  ...
  m_data.threshold_ssim =
      SETTING_GET(CFG_GROUP_FILTER, CFG_FILTER_THRESHOLD_SSIM).toDouble();
  m_data.batch_ssim =
      SETTING_GET(CFG_GROUP_FILTER, CFG_FILTER_BATCH_SSIM).toInt();
}
...
void WidgetFilter::save2Cfg() {
  ...
  SETTING_SET(CFG_GROUP_FILTER, CFG_FILTER_SSIM,
              QString::number(m_data.is_ssim));
  SETTING_SET(CFG_GROUP_FILTER, CFG_FILTER_THRESHOLD_SSIM,
              QString::number(m_data.threshold_ssim));
  SETTING_SET(CFG_GROUP_FILTER, CFG_FILTER_BATCH_SSIM,
