...
bool OnnxCheck::checkOnePicture(const QString &inputPath, const QString &outputPath) {
    ...
    cv::Mat resized_img;
    cv::resize(img, resized_img, cv::Size(m_info.model_width, m_info.model_height));
    cv::Mat blob = cv::dnn::blobFromImage(resized_img, 1 / 255.0, cv::Size(m_info.model_width, m_info.model_height), cv::Scalar(), true, false);
    m_net.setInput(blob);
    ...
    for (const auto& output : outputs) {
        ...
        int total_detections = output.size[1];  // num_anchors * grid_size * grid_size
    ...
    // NMS box
    cv::dnn::NMSBoxes(m_info.boxes, m_info.confidences, m_info.confidence_threshold, m_info.nms_threshold, m_info.nms_indices);
...
cv::Rect2d OnnxCheck::CoordinateTrans(const int &org_width, const int &org_height, const float &center_x, const float &center_y, const float &width, float &height) {
...
void OnnxCheck::doDataCheckout(const float *data, const int &img_width, const int &img_height) {
    ...
    cv::Rect2d box = CoordinateTrans(img_width, img_height, x_center, y_center, width, height);
    // get class score -> index 5...
    int class_id = std::max_element(data + 5, data + 5 + m_info.nc) - (data + 5);  // get nc count
...
bool OnnxCheck::doSaveImg(const cv::Mat &img, const QString& output_path) {
    ...
    QImage qimg(img_rgb.data, img.cols, img.rows, img.step, QImage::Format_RGB888);
...
cv::Mat OnnxCheck::doDrawBox(const cv::Mat &img) {
    ...
    for (int idx : m_info.nms_indices) {
        ...
        oss_cfd << std::fixed << std::setprecision(2) << m_info.confidences[idx] * 100 << "%";
        std::string confidence_str = oss_cfd.str();
        std::ostringstream oss_box;
        oss_box << std::fixed << std::setprecision(2) << "{" << box.x << ", " << box.y << ", " << box.width << ", " << box.height << "}";
        std::string box_info = oss_box.str();
        std::string info_base{label + ": " + confidence_str};
        int baseline = 0;
        cv::Size text_size = cv::getTextSize(info_base, cv::FONT_HERSHEY_SIMPLEX, 0.5, 2, &baseline);
        ...
        cv::putText(img_copy, info_base, pos_base, cv::FONT_HERSHEY_SIMPLEX, 0.5, COLORS.at(m_info.class_ids.at(idx)), 2);
        cv::putText(img_copy, info_box, pos_box, cv::FONT_HERSHEY_SIMPLEX, 0.5, COLORS.at(m_info.class_ids.at(idx)), 2);
    }
    return img_copy;
}
