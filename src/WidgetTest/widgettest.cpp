...
WidgetTest::WidgetTest(QWidget *parent)
    : QWidget(parent), ui(new Ui::WidgetTest) {
  ui->setupUi(this);
  initTest();
}
...
void WidgetTest::getCfgData() {
  m_data.model_height =
      SETTING_GET(CFG_GROUP_TEST, CFG_TEST_MODEL_HEIGHT).toInt();
  m_data.model_width =
      SETTING_GET(CFG_GROUP_TEST, CFG_TEST_MODEL_WIDTH).toInt();
  ...
  m_data.threshold_cfd =
      SETTING_GET(CFG_GROUP_TEST, CFG_TEST_THRESHOLD_CFD).toDouble();
  m_data.threshold_nms =
      SETTING_GET(CFG_GROUP_TEST, CFG_TEST_THRESHOLD_NMS).toDouble();
...
void WidgetTest::save2Cfg() {
  SETTING_SET(CFG_GROUP_TEST, CFG_TEST_MODEL_HEIGHT,
              QString::number(m_data.model_height));
  SETTING_SET(CFG_GROUP_TEST, CFG_TEST_MODEL_WIDTH,
  ...
  SETTING_SET(CFG_GROUP_TEST, CFG_TEST_THRESHOLD_CFD,
              QString::number(m_data.threshold_cfd));
  SETTING_SET(CFG_GROUP_TEST, CFG_TEST_THRESHOLD_NMS,
...
void WidgetTest::testOnnx() {
  ...
  QFileInfoList files = dir_input.entryInfoList(
      QStringList() << "*.jpg" << "*.png" << "*.jpeg", QDir::Files);

  for (const QFileInfo &fileInfo : files) {
    QString inputPath = fileInfo.absoluteFilePath();

    QString relativePath =
        fileInfo.absoluteFilePath().mid(m_data.dir_input.length());
    ...
    if (bOk) {
      WIDGET_LOG_INFO(
          QString("Operate success %1 => %2").arg(inputPath, outputPath));
    } else {
      WIDGET_LOG_WARN(
          QString("Operate failed %1 => %2").arg(inputPath, outputPath));
    }
    QApplication::processEvents();
  }
  WIDGET_LOG_INFO(
      QString("Travel director %1 finished!").arg(m_data.dir_input));
}
...
void WidgetTest::on_tableWidgetClassEdit_cellDoubleClicked(int row,
                                                           int column) {
  ui->tableWidgetClassEdit->editItem(
      ui->tableWidgetClassEdit->item(row, column));
}
