...
void MainWindow::initMain() {
  ...
  for (QAction *action : ui->menuTheme->actions()) {
    connect(action, &QAction::triggered, this,
            [this, action]() { menuThemeSelect(action); });
  }
  for (QAction *action : ui->menulanguage->actions()) {
    connect(action, &QAction::triggered, this,
            [this, action]() { menuLanguageSelect(action); });
  }
  getCfgData();
  show2Ui();
}
...
void MainWindow::addBasicWidget() {
  ...
  connect(&MY_WIDGET_LOG, &WidgetLog::signalHide, this,
          &MainWindow::slotWidgetTestHide);
}
  ...
  if (m_data.language.isEmpty()) {
    ...
    for (QAction *language : ui->menulanguage->actions()) {
      if (language->text() == m_data.language) {
        language->trigger();
        WIDGET_LOG_INFO(
            QString("use default language: %1").arg(language->text()));
        break;
      }
    }
  }
...
void MainWindow::on_checkBoxShowLog_checkStateChanged(
